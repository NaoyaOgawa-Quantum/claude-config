...
# 新規作成
cd ~/Claude
gh repo create NaoyaOgawa-Quantum/<name> --private --description "<English description>" --clone
cd <name> && git branch -M main
# 必須ファイル作成（§2参照）→ initial commit + push
git add . && git commit -m "Initial commit: <概要>" && git push -u origin main

# 未clone リポの同期
gh repo list NaoyaOgawa-Quantum --limit 50 --json name  # 一覧取得
gh repo clone NaoyaOgawa-Quantum/<name>                  # clone
```
...
- パス: `~/Claude/<name>/`
- ブランチ: `main`
- リモート: `NaoyaOgawa-Quantum/<name>` (private)
...
7. 自分（NaoyaOgawa-Quantum）のリポのみ操作。他ユーザーのリポは絶対に触らない
